    ...
    O: WireReady + Clone + Sync + 'static + Unpin,
{
    pub async fn server_setup(
        ...
        let incoming_conn_task = tokio::spawn(start_conn_all(node_addr.clone(), self.my_id));

        // Sleep for sometime until we are sure everyone is listening
        tokio::time::sleep(std::time::Duration::from_secs(5)).await;

        // Start connecting to other nodes
        let mut writers = outgoing_conn(self.my_id, &node_addr).await;
        ...
        tokio::spawn(protocol_event_loop(
            self.num_nodes,
            in_send,
            out_recv,
            unified_stream,
...
async fn start_conn_all(
    ...
    let listener = TcpListener::bind(&node_addr[&my_id])
        .await
